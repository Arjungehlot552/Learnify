...
const ProjectCards = () => {
  ...
  const projects = [
    { id: 1, title: "Computer Science Projects", img: i1 },
    { id: 2, title: "Artificial Intelligence Projects", img: i2 },
    { id: 3, title: "Civil Engineering Projects", img: i3 },
    { id: 4, title: "Data Science Projects", img: i4 },
    { id: 5, title: "Mechanical Engineering Projects", img: i5 },
    { id: 6, title: "Physics Projects", img: i6 },
    { id: 7, title: "Chemistry Projects", img: i7 }
  ];
      ...
      <div className="h-56 w-56 outline rounded-md outline-slate-400 relative group">
        <Link to={"/projectdisplay"}>
          ...
          <div className="absolute h-full w-full rounded-md bg-black/30 flex items-center justify-center -bottom-10 group-hover:bottom-0 opacity-0 group-hover:opacity-100 transition-all duration-200">
            <p className="text-white font-semibold text-xl text-center">{props.title}</p>
          </div>
        </Link>
      </div>
    );
    ...
    <div style={{boxShadow: "0 0 5px 0.5px gray"}} className="mt-12 p-4 shadow-xl rounded-xl">
      ...
      <div className="flex items-center justify-between gap-12 w-full p-4 mt-8">
        {projects.slice(0, 4).map((project) => (
          <Card key={project.id} title={project.title} img={project.img} />
        ))}
      </div>
